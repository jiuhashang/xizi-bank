<template>
  <div>
    <div class="shang">
      <h2 @click="$router.back()">&lt; {{ $route.meta.title }}</h2>
    </div>
    <el-card>
...
<script>
import { getProjectDetail, getProjectCount } from '@/api/bank'
...
export default {
  ...
  data() {
    ...
    created() {
      ...
      this.getProjectCount(this.projectId)
    },
    watch: {
    ...
    methods: {
      ...
      getProjectCount() {
        getProjectCount({ projectId: this.projectId}).then(res => { // 收益试算
          const { province, city, yearSunShine, mayInstallVolume, seProjectProfitConfig, projectTotalProfitModel } = res.data
